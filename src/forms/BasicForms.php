...
use xxAROX\forms\elements\Input;
...
class BasicForms{
	...
	function sendPublicCosmeticsForm(Player $player): void{
		...
		$player->sendForm(new MenuForm(
			...
			array_merge(($player->getSkin()->getSkinData() != $session->getLegacySkin()->getSkinData() ? [new Button("§cReset", function (Player $player) use ($session): void{
				$session->deactivateCosmetics();
				$player->setSkin($session->getLegacySkin());
				$player->sendSkin();
			})] : []), array_map(function (Cosmetic $cosmetic) use ($session){
				return new Button($cosmetic->getDisplayName(), function (Player $player) use ($cosmetic, $session): void{
	...
	function sendSlotCosmeticsForm(Player $player): void{
		...
		$player->sendForm(new MenuForm(
			...
			array_merge(($player->getSkin()->getSkinData() != $session->getLegacySkin()->getSkinData() ? [new Button("§cReset", function (Player $player) use ($session): void{
				...
				return new Button($cosmetic->getDisplayName() . "§r" . PHP_EOL . ($session->isActiveCosmetic($cosmetic) ? "§aActive" : "§cInactive"), function (Player $player) use ($cosmetic, $session): void{
					if (!$session->isActiveCosmetic($cosmetic)) {
						$session->activateCosmetic($cosmetic);
					} else {
						$session->deactivateCosmetic($cosmetic);
