...
use xxAROX\utils\addons\AddonManager;
use xxAROX\utils\addons\commando\CommandoAddon;
...
class CosmeticX extends PluginBase{
	...
	static string $URL = "cosmetic-x.de";
	...
	protected function onEnable(): void{
		...
		AddonManager::getInstance()->registerAddon(CommandoAddon::class);
	...
	private function loadCosmetics(): void{
		self::sendRequest(new ApiRequest("/cosmetics", [], true), function (array $data){
			foreach ($data as $obj) {
				CosmeticManager::getInstance()->registerCosmetic((string)$obj["id"], (string)$obj["category"], $obj["name"], $obj["owner"], $obj["display_name"], $obj["creator"], (isset($obj["image"]) ? new Image($obj["image"], str_starts_with($obj["image"], "http") ? Image::TYPE_URL : Image::TYPE_PATH) : null));
			}
