...
final class CosmeticManager{
	 ...
	 * @internal
	 */
	function registerCosmetic(string $id, string $category, string $name, string $owner, string $display_name, string $creator, ?Image $image = null): void{
		$cosmetic = new Cosmetic($id, $category, $name, $display_name, $creator, $owner, $image);
	 ...
	 * @param null|Closure $filter (Cosmetic $cosmetic): bool
	 * @return array<Cosmetic>
	 */
	function getCosmetics(Closure $filter = null): array{
		return array_filter(array_merge($this->publicCosmetics, $this->serverCosmetics), $filter);
	}
